...
const CATEGORIAS = ["Anillo", "Collar", "Pulsera", "Aretes", "Broche", "Reloj", "Otro"];
...
export default function App() {
  ...
  const stats = useMemo(() => {
    const total = clientes.reduce((acc, c) => acc + Number(c.precio || 0), 0);
    const promedio = clientes.length ? total / clientes.length : 0;
    const ultima = clientes.length
      ? [...clientes]
          .filter((c) => c.fecha)
          .sort((a, b) => String(b.fecha).localeCompare(String(a.fecha)))[0]?.fecha || "—"
      : "—";
  ...
  async function handleSave(e) {
    ...
    try {
      ...
      if (editingId) {
        ...
        setMensaje(`Cliente actualizado: ${payload.nombre}`);
      } else {
        const res = await fetch("/api/clientes", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        setMensaje(`Cliente guardado: ${payload.nombre}`);
      }
        ...
        <aside className="sidebar">
          <div className="brand-card">
            ...
            <p className="brand-text">
              Sistema web para controlar clientes y ventas de joyería conectado a Firebase Realtime Database desde el backend.
            </p>
          </div>
        ...
        <main className="content">
          <div className="topbar">
            ...
            <div className="topbar-actions">
              <button className="btn btn-secondary" onClick={() => downloadCSV(clientes)}>
                <Download size={16} />
                Exportar CSV
              </button>

              <button className="btn btn-secondary" onClick={cargarClientes}>
                <RefreshCw size={16} />
                Recargar
              </button>
          ...
          <div className="stats-grid">
            <StatCard label="Clientes" value={stats.clientes} subtitle="Registros totales" />
            <StatCard label="Ventas totales" value={money(stats.total)} subtitle="Suma acumulada" />
            <StatCard label="Ticket promedio" value={money(stats.promedio)} subtitle="Promedio por cliente" />
            <StatCard label="Última venta" value={stats.ultima} subtitle="Fecha más reciente" />
          </div>
          ...
          <div className="table-card">
            <div className="table-header">
              <div>
                ...
                <p>{loading ? "Cargando..." : `${filtrados.length} resultado(s) mostrados`}</p>
              </div>
                  ...
                  {!loading && filtrados.length === 0 ? (
                    ...
                    filtrados.map((c) => (
                      ...
                      >
                        ...
                        <td>{c.joya || "—"}</td>
                        <td>
                          <span className="category-pill">{c.categoria || "—"}</span>
                        </td>
          ...
          <div className="footer-note">
            Esta versión usa Firebase Admin únicamente en el backend de Vercel. Las credenciales no se exponen al navegador.
          </div>
        </main>
      </div>
    </div>
